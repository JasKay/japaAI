...
// const STAGE_READINESS: Record<number, number> = {
//   1: 0,    // Deciding to Move
//   2: 20,   // Preparing Documents
//   3: 40,   // Before Departure
//   4: 60,   // First 72 Hours
//   5: 80,   // First 30 Days
//   6: 100,  // Settling & Thriving
// }
...
export default function Dashboard() {
  ...
  const [visaPath, setVisaPath] = useState<VisaPath | null>(null)
  ...
  const getDepartureDateWarning = () => {
    ...
    const today = new Date()
    const daysUntil = Math.ceil((departureDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
    if (daysUntil < 0) return "⚠️ Your date is in the past. Update it."
    if (daysUntil < 14) return '⚠️ Very soon (< 2 weeks). Visa processing takes 3-8 weeks. Risky.'
    if (daysUntil < 30) return '⚠️ Tight timeline (< 1 month). Doable but stressful.'
    if (daysUntil < 60) return '✓ Good timeline. Enough time for most visas.'
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Fetch all visa paths and filter in JavaScript
        const { data: allVisaPaths } = await supabase
          .from('visa_paths')
          .select('*')

        console.log('DEBUG: All visa paths:', allVisaPaths)
        console.log('DEBUG: Looking for:', {
          home_country: userProfileData.home_country,
          destination_country: userProfileData.destination_country,
          visa_type_id: userProfileData.visa_type,
        })

        let visaPathData = null
        if (allVisaPaths) {
          visaPathData = allVisaPaths.find(vp => 
            vp.home_country === userProfileData.home_country &&
            vp.destination_country === userProfileData.destination_country &&
            vp.visa_type_id === userProfileData.visa_type
          )
        }

        console.log('DEBUG: Matching visa_path:', visaPathData)

        if (!visaPathData) {
          console.error('Visa path not found')
          setVisaPath(null)
          setTasks([])
        } else {
          setVisaPath(visaPathData)

          // Fetch tasks for this visa path
          const { data: visaPathTasksData } = await supabase
            .from('visa_path_tasks')
            .select('task_id')
            .eq('visa_path_id', visaPathData.id)

          console.log('DEBUG: visa_path_tasks:', visaPathTasksData)

          const taskIds = visaPathTasksData?.map(vpt => vpt.task_id) || []
          ...
          if (taskIds.length > 0) {
            ...
            console.log('DEBUG: tasks loaded:', tasksData?.length)
            setTasks(tasksData || [])
          } else {
            setTasks([])
          }
...
// })
  ...
  const daysUntilDeparture = Math.ceil(
    (new Date(userData.expected_departure).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
  )

  const selectedStageData = stages.find((s) => s.id === selectedStage)
  const stageTasks = tasks.filter((t) => t.stage_id === selectedStage)
  const completedCount = stageTasks.filter((t) => userTasks.get(t.id)?.status === 'completed').length
...
>
  {userData?.home_country} → {userData?.destination_country} | {visaPath?.name} | {userData?.expected_departure ? new Date(userData.expected_departure).toLocaleDateString() : 'Not set'} (edit)
</button>
            ...
            {stages.map((stage) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-gray-900 mb-2">{stage.name}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 sticky top-32">
              {selectedStageData && (
                <>
                  <h3 className="text-xl font-bold text-gray-900 mb-2">{selectedStageData.name}</h3>
                  <p className="text-sm text-gray-600 mb-4">{selectedStageData.description}</p>

                  <div className="bg-indigo-50 rounded-lg p-4 mb-4 border border-indigo-200">
                    <p className="text-xs font-medium text-indigo-700 mb-2">Stage Progress</p>
              ...
              <>
                {stageTasks.length === 0 ? (
                  <div className="bg-white rounded-2xl shadow-sm p-12 text-center border border-gray-100">
                    <p className="text-lg text-gray-500">No tasks for this stage yet.</p>
                  </div>
                ) : (
      ...
      <EditProfileModal
        ...
        currentData={{
          ...
          destination_country: userData?.destination_country || '',
